pipeline {
    ...
    stages {
        ...
        stage('package') {
            steps {
                sh "docker build -t 538471682716.dkr.ecr.ap-southeast-1.amazonaws.com/supriyadin-app-demo:${env.BUILD_NUMBER} ."
                sh "eval \$(aws ecr get-login --no-include-email --registry-ids 538471682716 --region ap-southeast-1)"
                sh "docker push 538471682716.dkr.ecr.ap-southeast-1.amazonaws.com/supriyadin-app-demo:${env.BUILD_NUMBER}"
